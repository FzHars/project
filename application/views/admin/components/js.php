...
<!-- <script>
    document.getElementById('hari_cuti').addEventListener('change', function() {
        const tglDiajukan = new Date(document.getElementById('tgl_diajukan').value);
        const hariCuti = parseInt(this.value);

        if (!isNaN(hariCuti)) {
            const tglBerakhir = new Date(tglDiajukan);
            tglBerakhir.setDate(tglBerakhir.getDate() + hariCuti);
            document.getElementById('berakhir').value = tglBerakhir.toISOString().split('T')[0]; // Update input berakhir
        }
    });
</script> -->
